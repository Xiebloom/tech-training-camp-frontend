      ...
      for (var i = 0, len = rows.length; i < len; i++) {
        ...
        if (matchArr) {
          ...
          switch (matchArr[0]) {
            ...
            case '# ':
              // console.log('#!')
              html += '<h1>' + this.compile([rows[i].substring(2)]) + '</h1>'
              break
            case '## ':
              html += '<h2>' + this.compile([rows[i].substring(3)]) + '</h2>'
              break
            case '### ':
              html += '<h3>' + this.compile([rows[i].substring(4)]) + '</h3>'
              break
            case '#### ':
            ...
            case '* ':
              ...
              re = /^\*\s/
              while (i < len && rows[i].match(re)) {
                temp += '<li>' + this.compile([rows[i].substring(2)]) + '</li>'
                if (rows[i + 1] && rows[i + 1].match(re)) i++
                else break
              }
              html += '<ul>' + temp + '</ul>'
